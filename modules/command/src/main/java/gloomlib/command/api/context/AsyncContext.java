package gloomlib.command.api.context;
...
public class AsyncContext extends GloomCommandContext {
    ...
    /**
     * Cached main-thread checker, resolved once at class load time.
     * On Folia: delegates to {@code RegionizedServer.isGlobalTickThread()} via reflected method handle.
     * On standard Paper/Spigot: delegates to {@link org.bukkit.Bukkit#isPrimaryThread()}.
     */
    private static final BooleanSupplier MAIN_THREAD_CHECKER;

    static {
        BooleanSupplier checker;
        try {
            java.lang.reflect.Method method = Class
                    .forName("io.papermc.paper.threadedregions.RegionizedServer")
            ...
            checker = org.bukkit.Bukkit::isPrimaryThread;
        }
        MAIN_THREAD_CHECKER = checker;
    }
     ...
     * On Folia, delegates to {@code RegionizedServer.isGlobalTickThread()}.
     * On standard Paper/Spigot, delegates to {@link org.bukkit.Bukkit#isPrimaryThread()}.
     *
     * <p>The underlying checker is resolved once at class load time via {@link #MAIN_THREAD_CHECKER},
     * so repeated calls incur no reflection overhead.</p>
     *
     * @return True if on Global Region thread
     */
    public boolean isMainThread() {
        return MAIN_THREAD_CHECKER.getAsBoolean();
    }
